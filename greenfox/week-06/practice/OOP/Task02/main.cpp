...
int main() {
    ...
    BlogPost blogPost3("William Turton", "\"One Engineer Is Trying to Get IBM to Reckon With Trump\"",
                       ...
                       "\"2017.03.28.\"");
    blog.addToList(blogPost1);
    blog.addToList(blogPost2);
    blog.addToList(blogPost3);
    for (int i = 0; i < blog.getBlogPostsSize(); ++i) {
        std::cout << blog.getBlogPost(i).getTitle() << " titled by " << blog.getBlogPost(i).getAuthorName() << " posted at "
                  << blog.getBlogPost(i).getPublicationDate() << "\n" << blog.getBlogPost(i).getText() << std::endl;
    }
    return 0;
}