...
void draw() {
    ...
    int i = 0;
    int plus = i * spacer;
    while (plus <= SCREEN_WIDTH) {
        if (plus <= SCREEN_HEIGHT) {
            SDL_SetRenderDrawColor(gRenderer, 0xFF /*R*/, 0x00 /*G*/, 0xFF /*B*/, 0xFF /*A*/);
            SDL_RenderDrawLine(gRenderer, plus, 0, SCREEN_WIDTH, plus);
        }
        SDL_SetRenderDrawColor(gRenderer, 0x00 /*R*/, 0xFF /*G*/, 0x00 /*B*/, 0xFF /*A*/);
        SDL_RenderDrawLine(gRenderer, 0, plus, plus, SCREEN_HEIGHT);
        ++i;
        plus = i * spacer;
    }
}
